...
const struct attm_desc diss_att_db[DIS_IDX_NB] =
{
    ...
    [DIS_IDX_SVC]                           =   {GATT_DECL_PRIMARY_SERVICE, PROP(RD), 0, 0},

    // Manufacturer Name Characteristic Declaration
    [DIS_IDX_MANUFACTURER_NAME_CHAR]        =   {GATT_DECL_CHARACTERISTIC, PROP(RD), 0, 0},
    // Manufacturer Name Characteristic Value
    [DIS_IDX_MANUFACTURER_NAME_VAL]         =   {ATT_CHAR_MANUF_NAME, PROP(RD), PERM(RI, ENABLE), DIS_VAL_MAX_LEN},

    // Model Number String Characteristic Declaration
    [DIS_IDX_MODEL_NB_STR_CHAR]             =   {GATT_DECL_CHARACTERISTIC, PROP(RD), 0, 0},
    // Model Number String Characteristic Value
    [DIS_IDX_MODEL_NB_STR_VAL]              =   {ATT_CHAR_MODEL_NB, PROP(RD), PERM(RI, ENABLE), DIS_VAL_MAX_LEN},

    // Serial Number String Characteristic Declaration
    [DIS_IDX_SERIAL_NB_STR_CHAR]            =   {GATT_DECL_CHARACTERISTIC, PROP(RD), 0, 0},
    // Serial Number String Characteristic Value
    [DIS_IDX_SERIAL_NB_STR_VAL]             =   {ATT_CHAR_SERIAL_NB, PROP(RD), PERM(RI, ENABLE), DIS_VAL_MAX_LEN},

    // Hardware Revision String Characteristic Declaration
    [DIS_IDX_HARD_REV_STR_CHAR]             =   {GATT_DECL_CHARACTERISTIC, PROP(RD), 0, 0},
    // Hardware Revision String Characteristic Value
    [DIS_IDX_HARD_REV_STR_VAL]              =   {ATT_CHAR_HW_REV, PROP(RD), PERM(RI, ENABLE), DIS_VAL_MAX_LEN},

    // Firmware Revision String Characteristic Declaration
    [DIS_IDX_FIRM_REV_STR_CHAR]             =   {GATT_DECL_CHARACTERISTIC, PROP(RD), 0, 0},
    // Firmware Revision String Characteristic Value
    [DIS_IDX_FIRM_REV_STR_VAL]              =   {ATT_CHAR_FW_REV, PROP(RD), PERM(RI, ENABLE), DIS_VAL_MAX_LEN},

    // Software Revision String Characteristic Declaration
    [DIS_IDX_SW_REV_STR_CHAR]               =   {GATT_DECL_CHARACTERISTIC, PROP(RD), 0, 0},
    // Software Revision String Characteristic Value
    [DIS_IDX_SW_REV_STR_VAL]                =   {ATT_CHAR_SW_REV, PROP(RD), PERM(RI, ENABLE), DIS_VAL_MAX_LEN},

    // System ID Characteristic Declaration
    [DIS_IDX_SYSTEM_ID_CHAR]                =   {GATT_DECL_CHARACTERISTIC, PROP(RD), 0, 0},
    // System ID Characteristic Value
    [DIS_IDX_SYSTEM_ID_VAL]                 =   {ATT_CHAR_SYS_ID, PROP(RD), PERM(RI, ENABLE), DIS_SYS_ID_LEN},

    // IEEE 11073-20601 Regulatory Certification Data List Characteristic Declaration
    [DIS_IDX_IEEE_CHAR]                     =   {GATT_DECL_CHARACTERISTIC, PROP(RD), 0, 0},
    // IEEE 11073-20601 Regulatory Certification Data List Characteristic Value
    [DIS_IDX_IEEE_VAL]                      =   {ATT_CHAR_IEEE_CERTIF, PROP(RD), PERM(RI, ENABLE), DIS_SYS_ID_LEN},

    // PnP ID Characteristic Declaration
    [DIS_IDX_PNP_ID_CHAR]                   =   {GATT_DECL_CHARACTERISTIC, PROP(RD), 0, 0},
    // PnP ID Characteristic Value
    [DIS_IDX_PNP_ID_VAL]                    =   {ATT_CHAR_PNP_ID, PROP(RD), PERM(RI, ENABLE), DIS_PNP_ID_LEN},
};
 ...
 ****************************************************************************************
 */
static uint8_t diss_init (prf_data_t* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct diss_db_cfg* params)
{
    ...
    {
        PRF_ENV_T(diss)* diss_env =
                (PRF_ENV_T(diss)* ) ke_malloc(sizeof(PRF_ENV_T(diss)), KE_MEM_ATT_DB);

        // allocate DISS required environment variable
        env->env = (prf_hdr_t*) diss_env;
 ...
 ****************************************************************************************
 */
static void diss_destroy(prf_data_t* env)
{
    PRF_ENV_T(diss)* diss_env = (PRF_ENV_T(diss)*) env->env;
 ...
 ****************************************************************************************
 */
static void diss_create(prf_data_t* env, uint8_t conidx)
{
    /* Nothing to do */
}
 ...
 ****************************************************************************************
 */
static void diss_cleanup(prf_data_t* env, uint8_t conidx, uint8_t reason)
{
    /* Nothing to do */
}
...
uint8_t diss_handle_to_value(PRF_ENV_T(diss)* env, uint16_t handle)
{
...
uint16_t diss_value_to_handle(PRF_ENV_T(diss)* env, uint8_t value)
{
