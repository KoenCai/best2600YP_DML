...
const struct attm_desc_128 sv_att_db[SMARTVOICE_IDX_NB] = {
    ...
    [SMARTVOICE_IDX_CMD_TX_VAL]     =   {sv_cmd_tx_char_val_uuid_128_content, PERM(NTF, ENABLE) | PERM(IND, ENABLE) | PERM(RD, ENABLE) | PERM(RP, AUTH), 
    ...
    [SMARTVOICE_IDX_CMD_TX_NTF_CFG] =   {ATT_DESC_CLIENT_CHAR_CFG_UUID, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(NP, AUTH) | PERM(WP, AUTH), 0, 0},
    ...
    [SMARTVOICE_IDX_DATA_TX_VAL]     =   {sv_data_tx_char_val_uuid_128_content, PERM(NTF, ENABLE) | PERM(IND, ENABLE) | PERM(RD, ENABLE) | PERM(RP, AUTH), 
    	PERM(RI, ENABLE) | PERM_VAL(UUID_LEN, PERM_UUID_128), AI_MAX_LEN},
    // Data TX Characteristic - Client Characteristic Configuration Descriptor
    [SMARTVOICE_IDX_DATA_TX_NTF_CFG] =   {ATT_DESC_CLIENT_CHAR_CFG_UUID, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(NP, AUTH), 0, 0},
	...
	uint16_t app_task, uint8_t sec_lvl, void* params)
{
    ...
    {
        app_btgatt_addsdp(ATT_SERVICE_UUID, *start_hdl, *start_hdl+SMARTVOICE_IDX_NB-1);
    }
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        sv_env->prf_env.prf_task   = env->task |
                    (PERM_GET(sec_lvl, SVC_MI) ? PERM(PRF_MI, ENABLE) : PERM(PRF_MI, DISABLE));
...
const ai_ble_prf_task_cbs_t smartvoice_itf =
{
