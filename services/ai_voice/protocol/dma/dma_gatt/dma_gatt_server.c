	...
	uint16_t app_task, uint8_t sec_lvl, void* params)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        dma_env->prf_env.prf_task   = env->task |
                    (PERM_GET(sec_lvl, SVC_MI) ? PERM(PRF_MI, ENABLE) : PERM(PRF_MI, DISABLE));
									  ...
									  ke_task_id_t const src_id)
{
	...
	PRF_ENV_T(ai) *dma_env = PRF_ENV_GET(AI, ai);
	dma_env->isCmdNotificationEnabled[conidx] = false;
	dma_env->isDataNotificationEnabled[conidx] = false;
    ...
    if (dma_env != NULL) {
		if (param->handle == (dma_env->shdl + DMA_IDX_CMD_TX_NTF_CFG)) {
            ...
            if (value == PRF_CLI_STOP_NTFIND) {
				...
				dma_env->isCmdNotificationEnabled[conidx] = true;
			} else {
            ...
            if (status == GAP_ERR_NO_ERROR) {
                ...
                struct ble_ai_tx_notif_config_t * ind = KE_MSG_ALLOC(DMA_CMD_CCC_CHANGED,
                        prf_dst_task_get(&dma_env->prf_env, conidx),
                        prf_src_task_get(&dma_env->prf_env, conidx),
                        ble_ai_tx_notif_config_t);

                ind->isNotificationEnabled = dma_env->isCmdNotificationEnabled[conidx];
            ...
            if (value == PRF_CLI_STOP_NTFIND) {
				...
				dma_env->isDataNotificationEnabled[conidx] = true;
			} else {
                status = PRF_APP_ERROR;
            }
            ...
            if (status == GAP_ERR_NO_ERROR) {
                //Inform APP of TX ccc change
                struct ble_ai_tx_notif_config_t * ind = KE_MSG_ALLOC(DMA_DATA_CCC_CHANGED,
                        prf_dst_task_get(&dma_env->prf_env, conidx),
                        prf_src_task_get(&dma_env->prf_env, conidx),
                        ble_ai_tx_notif_config_t);

                ind->isNotificationEnabled = dma_env->isDataNotificationEnabled[conidx];

                ke_msg_send(ind);
            }			
		} else if (param->handle == (dma_env->shdl + DMA_IDX_CMD_RX_VAL)) {
			//inform APP of data
            struct ble_ai_rx_data_ind_t * ind = KE_MSG_ALLOC_DYN(DMA_CMD_RECEIVED,
                    prf_dst_task_get(&dma_env->prf_env, conidx),
                    prf_src_task_get(&dma_env->prf_env, conidx),
			...
			//inform APP of data
            struct ble_ai_rx_data_ind_t * ind = KE_MSG_ALLOC_DYN(DMA_DATA_RECEIVED,
                    prf_dst_task_get(&dma_env->prf_env, conidx),
                    prf_src_task_get(&dma_env->prf_env, conidx),
                    ble_ai_rx_data_ind_t,
                    sizeof(struct ble_ai_rx_data_ind_t) + param->length);
                                 ...
                                 ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if ((GATTC_NOTIFY == param->operation) || (GATTC_INDICATE == param->operation)) {
        ...
        struct ble_ai_tx_sent_ind_t * ind = KE_MSG_ALLOC(DMA_TX_DATA_SENT,
                prf_dst_task_get(&dma_env->prf_env, conidx),
                prf_src_task_get(&dma_env->prf_env, conidx),
...
{	
    ...
    if (param->handle == (dma_env->shdl + DMA_IDX_CMD_TX_NTF_CFG)) {
		...
		if (dma_env->isCmdNotificationEnabled[conidx]) {
			notify_ccc = 1;
		} else {
			notify_ccc = 0;
		}
	...
	if (param->handle == (dma_env->shdl + DMA_IDX_DATA_TX_NTF_CFG)) {
		...
		if (dma_env->isDataNotificationEnabled[conidx]) {
			notify_ccc = 1;
		} else {
			notify_ccc = 0;
		}
...
static void send_notifiction(uint8_t conidx, uint8_t contentType, const uint8_t* ptrData, uint32_t length)
{
	...
	if ((AI_CONTENT_TYPE_COMMAND == contentType) && 
		(dma_env->isCmdNotificationEnabled[conidx])) {
		handle_offset = DMA_IDX_CMD_TX_VAL;
	} else if ((AI_CONTENT_TYPE_DATA == contentType) && 
		(dma_env->isDataNotificationEnabled[conidx])) {
