...
export NUTTX_ROOT := $(KBUILD_ROOT)/thirdparty/incubator-nuttx/nuttx
NUTTX_LIBS := $(filter-out $(NUTTX_ROOT)/staging/libproxies.a ,$(wildcard $(NUTTX_ROOT)/staging/*.a))
NUTTX_CFG_FILE := $(KBUILD_ROOT)/config/$(T)/defconfig
endif
...
IMAGE_LIB := $(addsuffix .a,$(basename $(IMAGE_FILE)))
...
ifeq ($(NUTTX_BUILD),1)
KBUILD_CPPFLAGS += -fno-builtin -DNUTTX_BUILD -D__NUTTX_SUPPORT__
NUTTX_INCLUDE :=  \
	-I$(NUTTX_ROOT)/include\

NUTTX_INCLUDE += -include $(KBUILD_ROOT)/platform/hal/hal_trace.h
C++_ONLY_FLAGS += -DCONFIG_WCHAR_BUILTIN $(NUTTX_INCLUDE) -std=c++17 -D__NuttX__ -nostdinc++
...
ifeq ($(ARM_CMNS),1)
IMAGE-deps += ../$(ARM_CMSE_TARGET)/$(ARM_CMSE_TARGET)_CMSE_Lib.o
...
archive-sdk-lib = ( /usr/bin/printf 'create $2\n\
  addmod $(subst $(space),$(comma),$(strip $(filter-out %.a,$(1))))\n\
  $(foreach o,$(filter %.a,$(1)),addlib $o\n)save\nend' | $(AR) -M )
$(IMAGE_LIB): $(IMAGE_FILE) FORCE
	@$(call archive-sdk-lib,$(IMAGE_INIT) $(IMAGE_MAIN) $(IMAGE_VER),$@)
	@$(Q) cp $@ lib$@
...
PHONY += $(clean-dirs) clean IMAGE-clean distclean
$(clean-dirs):
	$(Q)$(MAKE) $(clean)=$(patsubst _clean_%,%,$@)

IMAGE-clean:
	$(Q)$(call CMDRMFILE,$(IMAGE_FILE) $(IMAGE_MAP) \
		$(IMAGE_BIN) $(STR_BIN) $(IMAGE_HEX) $(IMAGE_RCF) $(IMAGE_LST) $(IMAGE_SEC) \
