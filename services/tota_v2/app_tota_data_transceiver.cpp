...
typedef struct{
    ...
    /* rx buff: 4k  */
    uint16_t    buffPos;
    uint8_t     rxBuff[MAX_DATA_LENGTH];
    uint8_t     readBuff[640];
}data_handle_t;



static data_handle_t dataHandle;
...
void data_handle_timeout_callback(void const *arg);
...
static handle_status_e __handle_with_data_write_flash(uint8_t * pdata, uint16_t dataLen)
{
    ...
    TOTA_LOG_DBG(2, "[__handle__] recv dataLen:%d, flushBytes:%d", dataLen, dataHandle.flushBytes);
    if (dataHandle.buffPos + dataLen < dataHandle.flushBytes)
    {
        TOTA_LOG_DBG(2, "[__handle__] buffPos:%d, dataLen:%d", dataHandle.buffPos, dataLen);
        memcpy(dataHandle.rxBuff+dataHandle.buffPos, pdata, dataLen);
        dataHandle.buffPos += dataLen;
        dataHandle.processedBytes += dataLen;
    }
    else
    {
        uint16_t maxBytesToCopy = dataHandle.flushBytes - dataHandle.buffPos;
        TOTA_LOG_DBG(2, "[__handle__] buffPos:%d, dataLen:%d", dataHandle.buffPos, maxBytesToCopy);
        memcpy(dataHandle.rxBuff+dataHandle.buffPos, pdata, maxBytesToCopy);
        dataHandle.buffPos += maxBytesToCopy;
        TOTA_LOG_DBG(2, "[__handle__] writeAddr:0x%x, flushBytes:%d", dataHandle.handleAddr, dataHandle.flushBytes);
        if (!write_flash(dataHandle.handleAddr, dataHandle.rxBuff, dataHandle.flushBytes))
        {
            TOTA_LOG_DBG(0, "[__handle__] error! how to deal with write flash error");
            return HANDLE_ERROR;
        }
        TOTA_LOG_DBG(0, "[__handle__] write flash success, update info");
        dataHandle.handleAddr       += dataHandle.flushBytes;
        dataHandle.flushBytes       = 4096 - (dataHandle.handleAddr%4096);
        dataHandle.buffPos          = 0;
        dataHandle.processedBytes   += dataLen;
        TOTA_LOG_DBG(2, "[__handle__] buffPos:%d, dataLen:%d", dataHandle.buffPos, dataLen-maxBytesToCopy);
        memcpy(dataHandle.rxBuff, pdata+maxBytesToCopy, dataLen-maxBytesToCopy);
    }
    if ( dataHandle.handleLength == dataHandle.processedBytes )
    {
        TOTA_LOG_DBG(2, "[__handle__] writeAddr:0x%x, flushBytes:%d", dataHandle.handleAddr, dataHandle.flushBytes);
        if (!write_flash(dataHandle.handleAddr, dataHandle.rxBuff, dataHandle.buffPos))
        {
            TOTA_LOG_DBG(0, "[__handle__] error! how to deal with write flash error");
            reset_data_handle();
            return HANDLE_ERROR;
...
static handle_status_e __handle_with_data_read_flash(uint8_t * pdata, uint16_t dataLen)
{
    ...
    {   
        ...
        memcpy(dataHandle.readBuff, (uint8_t*)readAddr, readLen);
        TOTA_LOG_DBG(0, "[__handle__] >>>>>send-data>>>>>");
        app_tota_send(dataHandle.readBuff, readLen, OP_TOTA_DATA);
...
static void __read_flash_one_time(uint32_t readAddr, uint32_t readLen)
{
    ...
    while ( readLen > 640 )
    {
        ...
        memcpy(dataHandle.readBuff, (uint8_t*)readAddr, readMax);
        app_tota_send(dataHandle.readBuff, readMax, OP_TOTA_DATA_ENCODE);
        TOTA_LOG_DBG(0, "[__handle__] >>>>>send-data>>>>>");
        readAddr += readMax;
        readLen -= readMax;
    }
    ...
    memcpy(dataHandle.readBuff, (uint8_t*)readAddr, readLen);
    app_tota_send(dataHandle.readBuff, readLen, OP_TOTA_DATA_ENCODE);
    TOTA_LOG_DBG(0, "[__handle__] >>>>>send-data>>>>>");
}