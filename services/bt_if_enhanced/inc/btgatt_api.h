...
void btif_btgatt_disconnect_by_conidx(uint8_t condix);
uint32_t btif_btgatt_get_tx_mtu(uint8_t conidx);
...
bool btif_btgatt_is_connected_by_connhdl(uint16_t connhdl, uint8_t *conidx);
int32_t btif_btgatt_get_l2cap_buffer_size(uint8_t conidx);
void btif_btgatt_get_device_address(uint8_t conidx,uint8_t *addr);

void btif_btgatt_callback(uint8_t device_id, BtgattChannel *Chan, BtgattCallbackParms *Info, uint8_t reason);
