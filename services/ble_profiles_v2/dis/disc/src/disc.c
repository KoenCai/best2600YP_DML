 ...
 ****************************************************************************************
 */
static uint8_t disc_init (prf_data_t* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  void* params)
{
    uint8_t idx;
    //-------------------- allocate memory required for the profile  ---------------------

    PRF_ENV_T(disc)* disc_env =
            (PRF_ENV_T(disc)* ) ke_malloc(sizeof(PRF_ENV_T(disc)), KE_MEM_ATT_DB);

    // allocate DISC required environment variable
    env->env = (prf_hdr_t*) disc_env;
 ...
 ****************************************************************************************
 */
static void disc_destroy(prf_data_t* env)
{
    uint8_t idx;
    PRF_ENV_T(disc)* disc_env = (PRF_ENV_T(disc)*) env->env;
 ...
 ****************************************************************************************
 */
static void disc_create(prf_data_t* env, uint8_t conidx)
{
 ...
 ****************************************************************************************
 */
static void disc_cleanup(prf_data_t* env, uint8_t conidx, uint8_t reason)
{
    PRF_ENV_T(disc)* disc_env = (PRF_ENV_T(disc)*) env->env;
...
void disc_enable_rsp_send(PRF_ENV_T(disc) *disc_env, uint8_t conidx, uint8_t status)
{
