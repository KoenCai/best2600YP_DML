...
typedef enum
{
    ...
    OTA_GET_LOCAL_RD_RSQ = 0x9B,
    OTA_GET_LOCAL_RD_RSP = 0x9C,
...
typedef struct
{
    ...
    union {
        ...
        struct
        {
            uint8_t localRandom[2];
        } F_PACK rspLocalRandom;
    } payload;
...
extern void app_ibrt_ota_bp_check_cmd_send_handler(uint16_t rsp_seq, uint8_t *p_buff, uint16_t length);
