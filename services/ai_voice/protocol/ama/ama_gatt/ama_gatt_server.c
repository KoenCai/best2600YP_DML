...
const struct attm_desc_128 ama_att_db[AMA_IDX_NB] =
{
    ...
    [AMA_IDX_TX_VAL]     =   {ama_tx_char_val_uuid_128_content, PERM(NTF, ENABLE) | PERM(RD, ENABLE) | PERM(RP, AUTH), 
        PERM(RI, ENABLE) | PERM_VAL(UUID_LEN, PERM_UUID_128), AI_MAX_LEN},
    // Command TX Characteristic - Client Characteristic Configuration Descriptor
    [AMA_IDX_TX_NTF_CFG] =   {ATT_DESC_CLIENT_CHAR_CFG_UUID, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(NP, AUTH), 0, 0},
    ...
    uint16_t app_task, uint8_t sec_lvl, void* params)
{
    ...
    TRACE(2,"attm_ama_create_db_128 returns %d start handle is %d", status, *start_hdl);
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        ama_env->prf_env.prf_task   = env->task |
                    (PERM_GET(sec_lvl, SVC_MI) ? PERM(PRF_MI, ENABLE) : PERM(PRF_MI, DISABLE));
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    PRF_ENV_T(ai) *ama_env = PRF_ENV_GET(AI, ai);
    ama_env->isCmdNotificationEnabled[conidx] = false;
...
{   
    ...
    TRACE(3,"gattc_write_req_ind_handler ama_env %p write handle %d shdl %d", 
    ...
    if (ama_env != NULL)
    {
        if (param->handle == (ama_env->shdl + AMA_IDX_TX_NTF_CFG))
        {
            ...
            {
                ama_env->isCmdNotificationEnabled[conidx] = true;
            }
            else
            {
            ...
            if (status == GAP_ERR_NO_ERROR)
            {
                ...
                struct ble_ai_tx_notif_config_t * ind = KE_MSG_ALLOC(AMA_CCC_CHANGED,
                        prf_dst_task_get(&ama_env->prf_env, conidx),
                        prf_src_task_get(&ama_env->prf_env, conidx),
                        ble_ai_tx_notif_config_t);

                ind->isNotificationEnabled = ama_env->isCmdNotificationEnabled[conidx];
        ...
        {
            //inform APP of data
            struct ble_ai_rx_data_ind_t * ind = KE_MSG_ALLOC_DYN(AMA_RECEIVED,
                    prf_dst_task_get(&ama_env->prf_env, conidx),
                    prf_src_task_get(&ama_env->prf_env, conidx),
                                 ...
                                 ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if ((GATTC_NOTIFY == param->operation) || (GATTC_INDICATE == param->operation))
    {
        ...
        struct ble_ai_tx_sent_ind_t * ind = KE_MSG_ALLOC(AMA_TX_DATA_SENT,
                prf_dst_task_get(&ama_env->prf_env, conidx),
                prf_src_task_get(&ama_env->prf_env, conidx),
...
{   
    ...
    TRACE(2,"gattc_read_req_ind_handler read handle %d shdl %d", param->handle, ama_env->shdl);
    ...
    if (param->handle == (ama_env->shdl + AMA_IDX_TX_NTF_CFG))
    {
        ...
        if (ama_env->isCmdNotificationEnabled[conidx])
        {
            notify_ccc = 1;
        }
        else
